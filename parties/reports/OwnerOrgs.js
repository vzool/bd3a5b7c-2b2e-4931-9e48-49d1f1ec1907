...
app.controller('ownerOrgs', ['$scope', 'DTOptionsBuilder', 'DTColumnBuilder', '$http', 'CRUDServices', '$base64', '$routeParams', 'setting', '$location', 'growl', '$uibModal', function ($scope, DTOptionsBuilder, DTColumnBuilder, $http, CRUDServices, $base64, $routeParams, setting, $location, growl, $uibModal) {
$scope.owner={
    "name": "",
    "nationality": "",
    "phone": ""
...
//        $scope.getOwnerBeans();
        ...
        if ($routeParams.ownerID != null) {
            console.log($routeParams.ownerID);
            getOwner($routeParams.ownerID);
        }
...
//            $scope.resetFields();
        ...
        $scope.dtColumns = [
            ...
            DTColumnBuilder.newColumn(null).withTitle('الأحداث').notSortable().renderWith(function (data, type, full, meta) {
                var id = full["id"];
                return '<a href="#/orghist/' + full["id"] + '" class="btn btn-primary btn-xs"><i class="fa fa-edit"></i> View </a>';
            }),
        ...
        open = function (id) {
            ...
            CRUDServices.getData("report/orgnization_history", search).success(function (data) {
